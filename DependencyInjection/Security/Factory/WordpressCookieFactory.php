...
class WordpressCookieFactory implements SecurityFactoryInterface
{
    public function create(ContainerBuilder $container, $id, $config, $userProvider, $defaultEntryPoint)
    {
        ...
        $container
            ->setDefinition($providerId, new DefinitionDecorator('wordpress.security.authentication.provider.cookie'))
            ->replaceArgument(1, new Reference($userProvider));

        $listenerId = 'wordpress.security.authentication.listener.cookie.' . $id;
        $container
            ->setDefinition($listenerId, new DefinitionDecorator('wordpress.security.authentication.listener.cookie'))
